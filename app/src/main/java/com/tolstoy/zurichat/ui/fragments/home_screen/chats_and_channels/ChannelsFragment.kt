...
class ChannelsFragment : Fragment(R.layout.fragment_channels) {
     ...
     */
    private fun addHeaders(user: User) {
        val newList: ArrayList<ChannelModel> = ArrayList()

        val unreadList: ArrayList<ChannelModel> = ArrayList()
        val unreadChannelHeader = ChannelModel(
            getString(R.string.unread_messages),
            false,
            false,
            "channel_header_unread",
            generateRandomLong().toString(),
            0
        )

        val readList: ArrayList<ChannelModel> = ArrayList()
        val addChannelHeader = ChannelModel(
            getString(R.string._add_channel),
            false,
            false,
            "channel_header_add",
            generateRandomLong().toString(),
        ...
        val dividerHeader =
            ChannelModel("", false, false, "channel_header_add", generateRandomLong().toString(), 2)
        ...
        if (unreadList.size > 0) {
            ...
            for (channel in unreadList) {
                ...
                if (!(channel.name == getString(R.string.unread_messages) || channel.name == getString(
                        R.string.channels_
                    ))
                ) {
                    newList.add(channel)
     ...
     */
    private fun getListOfChannels(user: User) {
